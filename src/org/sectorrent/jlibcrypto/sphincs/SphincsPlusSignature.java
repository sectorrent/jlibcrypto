...
public class SphincsPlusSignature extends SignatureSpi {
    ...
    protected void engineInitVerify(PublicKey publicKey)throws InvalidKeyException {
        ...
        key = ((SphincsPlusPublicKey) publicKey).getPublicKeyBytes();
    ...
    protected void engineInitSign(PrivateKey privateKey)throws InvalidKeyException {
        ...
        key = ((SphincsPlusPrivateKey) privateKey).getPrivateKeyBytes();
        forSigning = true;
    }
    ...
    protected boolean engineVerify(byte[] sigBytes)throws SignatureException {
        ...
        byte[] m = Arrays.copyOfRange(sigBytes, SPX_BYTES, sigBytes.length);
        int sigOffset = 0;

        if(sigBytes.length <= SPX_BYTES){
        ...
        hashMessage(mhash, tree, idxLeaf, sigBytes, key, m);
        sigOffset += SPX_N;

        TreeHash.setTreeAddress(wotsAddr, tree[0]);
        AddressUtils.setKeyPairAddress(wotsAddr, idxLeaf[0]);

        forsPkFromSig(root, sigBytes, sigOffset, mhash, ctx, wotsAddr);
        sigOffset += SPX_FORS_BYTES;
        ...
        for(int i = 0; i < SPX_D; i++){
            ...
            Wots.wotsPkFromSig(wotsPk, sigBytes, sigOffset, root, ctx, wotsAddr);
            sigOffset += SPX_WOTS_BYTES;

            TreeHash.treeHash(leaf, 0, wotsPk, 0, SPX_WOTS_LEN, ctx, wotsPkAddr);

            computeRoot(root, 0, leaf, idxLeaf[0], 0, sigBytes, sigOffset, SPX_TREE_HEIGHT, ctx, treeAddr);
        ...
        return Arrays.equals(Arrays.copyOfRange(sigBytes, SPX_BYTES, sigBytes.length), message);
    }
