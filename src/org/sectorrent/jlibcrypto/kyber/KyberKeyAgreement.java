...
public class KyberKeyAgreement extends KeyAgreementSpi {
    ...
    private byte[] rnd = new byte[KyberParams.PARAMS_SYM_BYTES];
    ...
    protected byte[] engineGenerateSecret()
            throws IllegalStateException {
        byte[] sharedSecret = new byte[KyberParams.PARAMS_SYM_BYTES];
        try {
    ...
    private KyberDecrypted decrypt512(KyberCipherText kyberCiphertext) throws NoSuchAlgorithmException {
        ...
        byte[] subKr = new byte[kr.length - KyberParams.PARAMS_SYM_BYTES];
        System.arraycopy(kr, KyberParams.PARAMS_SYM_BYTES, subKr, 0, subKr.length);
        ...
        int index = KyberParams.KYBER_512SK_BYTES - KyberParams.PARAMS_SYM_BYTES;
    ...
    private byte[] verifyVariant(byte[] variant) throws IllegalArgumentException {
        if (variant.length > KyberParams.PARAMS_SYM_BYTES) {
            throw new IllegalArgumentException("Byte array exceeds allowable size of " + KyberParams.PARAMS_SYM_BYTES + " bytes");
        } else if (variant.length < KyberParams.PARAMS_SYM_BYTES) {
            byte[] tempData = new byte[KyberParams.PARAMS_SYM_BYTES];
            System.arraycopy(variant, 0, tempData, 0, variant.length);
            byte[] emptyBytes = new byte[KyberParams.PARAMS_SYM_BYTES - variant.length];
